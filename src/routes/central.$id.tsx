...
function CentralWizard() {
  ...
  async function load() {
    ...
    const [{ data: b }, { data: its }, { data: sup }, { data: resps }] = await Promise.all([
      ...
      supabase.from("bid_supplier_responses").select("id, supplier_id, observations, global_discount_type, global_discount_value").eq("bid_id", id),
    ]);
    setBid(b as Bid);
  ...
  function custoTotal(p?: Price, respId?: string): number {
    ...
    if (r && r.global_discount_value) {
      ...
      else v = Math.max(0, v - (r.global_discount_value / items.length)); // Aproximação de rateio se for fixo global
    }
  ...
  // ============== TOTAIS - CONSUMINDO ENGINE ==============
  const totals = useMemo(() => {
    // Se temos dados consolidados no header, usamos eles como fonte primária
    if (bid && bid.total_estimated !== undefined) {
      const rows = items.map((it) => {
        const c = Number(it.quoted_value) || 0;
        ...
        const chosen = it.chosen_response_id;
        const supplier = chosen ? suppliers.find((s) => s.id === responses.find((r) => r.id === chosen)?.supplier_id) : null;
        return { it, chosen, p: undefined, custo: c, final, lucroItem, totItemCusto, totItemFinal, editalUnit, totItemEdital, supplier };
      });

      return { 
        rows, 
        totalCusto: bid.total_quoted || 0, 
        totalFinal: bid.total_dispute || 0, 
        lucro: bid.total_profit_real || 0, 
        totalEdital: bid.total_estimated || 0, 
        margemMedia: bid.total_margin_real_pct || 0 
      };
    }

    // Fallback para cálculo local se o engine ainda não consolidou
    ...
    const rows = items.map((it) => {
      ...
      const editalUnit = Number(it.estimated_value) || 0;
      const totItemEdital = editalUnit * qtd;
      totalCusto += totItemCusto; totalFinal += totItemFinal; lucro += lucroItem;
  ...
  const finalSummary = useMemo(() => {
    if (!bid || (bid.resultado as ResultadoStatus | null) !== "ganha") return null;
  ...
  const propostaGerada = bid.status === "gerada" || bid.status === "finalizada";
  ...
  return (
    <AppShell title={`Cotação · ${bid.orgao || bid.processo || "Wizard"}`}>
      <div className="space-y-4">
        ...
        {(() => {
          // Se a licitação já estiver ganha, usamos o resumo final (homologado)
          // Caso contrário, usamos os totais da proposta (disputa)
          const isGanha = bid?.resultado === "ganha";
          
          const potentialTotal = bid?.total_dispute || 0;
          const winTotal = isGanha && finalSummary ? finalSummary.faturamento : (bid?.total_homologated || 0);
          const winCost = isGanha && finalSummary ? finalSummary.custo : ((bid?.total_homologated || 0) - (bid?.total_profit_real || 0));
