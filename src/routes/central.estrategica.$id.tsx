...
function VisaoEstrategica() {
  ...
  // O cabeçalho executivo agora vem diretamente do Engine (bid state)
  const executiveStats = useMemo(() => {
    return {
      totalEdital: bid?.total_estimated || 0,
      totalDisputa: bid?.total_dispute || 0,
      totalGanhando: bid?.total_homologated || 0,
      totalVenda: bid?.total_dispute || 0, // Durante a disputa, o total_dispute é a nossa projeção de venda
      lucroGanhando: bid?.total_profit_real || 0,
      margemGanhando: bid?.total_margin_real_pct || 0,
      statusRisco: (bid?.total_margin_real_pct || 0) < 15 ? 'critico' : (bid?.total_margin_real_pct || 0) < 25 ? 'atencao' : 'seguro'
    };
  }, [bid]);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 animate-in fade-in slide-in-from-top-4 duration-500">
          ...
          <Card className="bg-cyan-50 border-cyan-200">
            <CardContent className="p-4 flex flex-col items-center justify-center text-center">
              ...
              <p className="text-[10px] uppercase font-bold text-cyan-600 leading-none mb-1">Lucro (Ganhos)</p>
